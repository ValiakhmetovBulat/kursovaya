                ...
                if (Stream.CanRead)
                {
                    ...
                    else if (complexMessage.NumberStatus == 1)
                    {
                        using (EntityModelContainer db = new EntityModelContainer())
                        {
                            ...
                            {
                                if (db.UserSet.ToList()[i].Login == Convert.ToString(SerializeAndDeserialize.Deserialize(complexMessage.First)) &&
                                db.UserSet.ToList()[i].Password == Convert.ToString(SerializeAndDeserialize.
                                Deserialize(complexMessage.Second)))
                                {
                                    ...
                                    {
                                        Professor professor1 = db.UserSet.ToList()[i].Professor;
                                        Professor professor2 = professor1;
                                        m2 = SerializeAndDeserialize.Serialize(professor1);
                                    }
                                    cm.First = m1;
                                    cm.Second = m2;
                                    cm.NumberStatus = 2;
                            ...
                            cm.NumberStatus = 3;
                            m = SerializeAndDeserialize.Serialize(cm); Stream.Write(responseData, 0, responseData.Length);
                            label:
                            responseData = null;
                        }
                    }
